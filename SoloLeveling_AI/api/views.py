...
def request_ai(request):
    ...
    if request.method == "POST":
        ...
        try:
            request_data = models.Requests_history.objects.get(user_id=user).data
            data = [i for i in json.loads(request_data)['data']]
            # data.append(user_message)
            history, assistant_answer = openAI(data, user_message)

            # todo замена
            request_data = models.Requests_history.objects.get(user_id=user)
            ...
            request_data = models.Requests_history.objects.create(user_id=user,
